...
*Age/Expirience                   :   Age
*Measure of Sector Size           :   Value Added at Fixed Prices (KM Value Added)
*Results Reported in              :   Table 7 row (ii)   

*The Between-Industry Contribution Computed Here Corresponds to Line (ii) in Table 7 of the paper
*if 1977 is chosen as the initial year, 2005 as the final year, and if the HS Industries are those in the 
*benchmark set
...
* Start a Log File
log using Table7_row_ii, replace
...
*-------------------------------------------------------------------------------
* Import the Time Series of the Value Added Share of the HS Sector and Chain Price Indices 
* and save them in an Auxiliary File
use    $dir/Data_for_Calibration/Data_for_Calibration.dta
keep   if Year >= $Initial_Year & Year <= $Final_Year
keep   Year P_chain_HS P_chain_LS share_VA_HS share_VA_LS
rename share_VA_HS share_VA_HS_sector  
rename share_VA_LS share_VA_LS_sector 

gen rel_P_HS_LS                = P_chain_HS/P_chain_LS
gen rel_VA_LS_HS               = (1/share_VA_HS_sector)-1
gen rel_Q_LS_HS                = rel_VA_LS_HS*rel_P_HS_LS
gen share_VA_HS_sector_Pfixed  = 1/(1+ rel_Q_LS_HS)
gen share_VA_LS_sector_Pfixed  = 1-share_VA_HS_sector_Pfixed

keep   Year share_VA_HS_sector share_VA_LS_sector share_VA_HS_sector_Pfixed share_VA_LS_sector_Pfixed
...
merge m:1 Year using dta_files/VA_shares.dta 
replace share_VA_HS_sector_Pfixed = share_VA_LS_sector_Pfixed if sector == 2

rename share_VA_HS_sector_Pfixed share_VA_KM_sector 
...
keep Year sector share_eff_KM_HS share_VA_KM_sector avg_share_eff_KM_HS avg_alpha_j_HS_KM

reshape wide  share_eff_KM_HS share_VA_KM_sector avg_share_eff_KM_HS avg_alpha_j_HS_KM, i(Year) j(sector) 

rename avg_share_eff_KM_HS1 avg_share_eff_KM_HS
drop   avg_share_eff_KM_HS2

rename share_VA_KM_sector1 share_VA_KM_HS_sector 
rename share_VA_KM_sector2 share_VA_KM_LS_sector 
...
gen share_VA_KM_LS_sector_initial  = share_VA_KM_LS_sector  if Year == $Initial_Year
gen share_VA_KM_LS_sector_final    = share_VA_KM_LS_sector  if Year == $Final_Year
gen share_VA_KM_HS_sector_initial  = share_VA_KM_HS_sector  if Year == $Initial_Year
gen share_VA_KM_HS_sector_final    = share_VA_KM_HS_sector  if Year == $Final_Year
...
gen Delta_E_HS_sector_KM = share_VA_KM_HS_sector_final - share_VA_KM_HS_sector_initial
gen Delta_E_LS_sector_KM = share_VA_KM_LS_sector_final - share_VA_KM_LS_sector_initial
