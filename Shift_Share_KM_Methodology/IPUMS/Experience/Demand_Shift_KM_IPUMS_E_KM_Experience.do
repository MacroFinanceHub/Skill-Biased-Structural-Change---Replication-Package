...
*Generate Employment (Measured as Total Hours Worked)
...
collapse (sum) hours, by(year ind_eu31 sex educ_group exp_group)
egen id = concat(year sex educ_group exp_group), punct("_")
destring id, replace

*Merge the KM Wage Data
merge m:m id using Experience/KM_wage_data.dta
drop id _merge
...
bys  sex educ_group exp_group: egen avg_eff_weights = mean(eff_weights)
...
disp Delta
*-------------------------------------------------------------------------------
rm Experience/IPUMS_SomeCollege_bridged.dta
rm Experience/IPUMS_HS_bridged.dta
